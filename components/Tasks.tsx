    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-40 flex items-center justify-center" aria-modal="true" role="dialog">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg relative dark:bg-neutral-800">
        <div className="flex justify-between items-center pb-4 border-b dark:border-neutral-700">
          <h2 className="text-xl font-semibold text-neutral-800 dark:text-neutral-200">{isEditing ? 'Edit Task' : 'Add New Task'}</h2>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4 max-h-[70vh] overflow-y-auto pr-2">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label htmlFor="status" className="block text-sm font-medium text-neutral-700 dark:text-neutral-300">Status</label>
              <select
                id="status"
                value={status}
          ...
          <div>
            <label htmlFor="contactId" className="block text-sm font-medium text-neutral-700 dark:text-neutral-300">Assign to Contact</label>
            <select
              id="contactId"
              value={contactId}
           ...
           <div>
            <label htmlFor="dependencies" className="block text-sm font-medium text-neutral-700 dark:text-neutral-300">Dependencies (must be completed first)</label>
            <select
              id="dependencies"
              multiple
              value={dependencyIds}
              onChange={handleDepsChange}
              className="mt-1 block w-full h-32 px-3 py-2 border border-neutral-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-brand-primary focus:border-brand-primary sm:text-sm dark:bg-neutral-700 dark:border-neutral-600 dark:text-white"
            >
              {allTasks
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center" aria-modal="true" role="dialog">
            <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md relative dark:bg-neutral-800">
                ...
                <div className="mt-4">
                    <p className="text-sm text-neutral-700 dark:text-neutral-300">This task cannot be marked as 'Done' because the following prerequisite tasks are not complete:</p>
                    <ul className="list-disc list-inside mt-2 space-y-1 text-sm text-neutral-600 bg-neutral-50 p-3 rounded-md dark:bg-neutral-700 dark:text-neutral-300">
                        {incompleteDeps.map(task => (
                            <li key={task.id}><strong>{task.title}</strong> (Status: {task.status})</li>
                        ))}
                    </ul>
                    <p className="text-sm text-neutral-700 mt-4 dark:text-neutral-300">Are you sure you want to proceed?</p>
                </div>
                <div className="flex justify-end pt-6 border-t mt-6 space-x-3 dark:border-neutral-700">
                    <button onClick={onClose} className="px-4 py-2 bg-neutral-100 text-neutral-700 rounded-md hover:bg-neutral-200 dark:bg-neutral-700 dark:text-neutral-200 dark:hover:bg-neutral-600">
                        Cancel
                    </button>
...
const TaskCard: React.FC<{ task: Task; onEdit: (task: Task) => void; onDelete: (taskId: string) => void; taskMap: Record<string, Task> }> = ({ task, onEdit, onDelete, taskMap }) => {
    ...
    return (
        <div className={`bg-white p-4 rounded-lg shadow mb-4 border-l-4 ${statusColors[task.status]} dark:bg-neutral-800`}>
            <div className="flex justify-between items-start">
              <p className="font-semibold text-gray-800 pr-2 dark:text-neutral-200">{task.title}</p>
            ...
            {task.description && <p className="text-sm text-gray-600 mt-1 dark:text-neutral-400">{task.description}</p>}
            <div className="flex justify-between items-end mt-4">
                ...
                <div className="text-right">
                    ...
                    <span className="block text-xs font-medium text-gray-500 dark:text-neutral-400">{new Date(task.dueDate).toLocaleDateString()}</span>
                </div>
            </div>
        </div>
    );
...
const TaskColumn: React.FC<{ title: TaskStatus; tasks: Task[]; onEdit: (task: Task) => void; onDelete: (taskId: string) => void; taskMap: Record<string, Task> }> = ({ title, tasks, onEdit, onDelete, taskMap }) => {
    return (
        <div className="flex-shrink-0 w-80 bg-gray-100 rounded-lg p-3 dark:bg-neutral-900">
            <div className="flex justify-between items-center mb-4">
                <h3 className="font-semibold text-gray-700 dark:text-neutral-300">{title} ({tasks.length})</h3>
            </div>
...
const Tasks: React.FC = () => {
    ...
    const ViewSwitcher: React.FC = () => (
      <div className="flex items-center p-1 bg-gray-200 rounded-lg dark:bg-neutral-700">
          <button onClick={() => setViewMode('table')} className={`px-3 py-1 text-sm font-medium rounded-md flex items-center ${viewMode === 'table' ? 'bg-white text-brand-primary shadow dark:bg-neutral-800' : 'text-gray-600 dark:text-neutral-300'}`}>
              <TableIcon className="w-4 h-4 mr-2" /> Table
          </button>
          <button onClick={() => setViewMode('board')} className={`px-3 py-1 text-sm font-medium rounded-md flex items-center ${viewMode === 'board' ? 'bg-white text-brand-primary shadow dark:bg-neutral-800' : 'text-gray-600 dark:text-neutral-300'}`}>
    ...
    return (
        <div className="p-8 h-full flex flex-col">
            ...
            <div className="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200 dark:bg-neutral-800 dark:border-neutral-700">
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    <select name="status" value={filters.status} onChange={handleFilterChange} className="w-full border-gray-300 rounded-md shadow-sm dark:bg-neutral-700 dark:border-neutral-600 dark:text-white">
                        <option value="">All Statuses</option>
                        {Object.values(TaskStatus).map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                    <select name="contactId" value={filters.contactId} onChange={handleFilterChange} className="w-full border-gray-300 rounded-md shadow-sm dark:bg-neutral-700 dark:border-neutral-600 dark:text-white">
                        <option value="">All Contacts</option>
                        {contacts.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                    </select>
                    <input type="date" name="startDate" value={filters.startDate} onChange={handleFilterChange} className="w-full border-gray-300 rounded-md shadow-sm dark:bg-neutral-700 dark:border-neutral-600 dark:text-white" placeholder="Start Date" />
                    <input type="date" name="endDate" value={filters.endDate} onChange={handleFilterChange} className="w-full border-gray-300 rounded-md shadow-sm dark:bg-neutral-700 dark:border-neutral-600 dark:text-white" placeholder="End Date" />
                    <button onClick={clearFilters} className="px-4 py-2 bg-neutral-200 text-neutral-700 rounded-md hover:bg-neutral-300 dark:bg-neutral-700 dark:text-neutral-200 dark:hover:bg-neutral-600">Clear Filters</button>
                </div>
            </div>
              ...
              <div className="bg-white rounded-lg shadow-md overflow-hidden dark:bg-neutral-800">
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
                      <thead className="bg-gray-50 dark:bg-neutral-700">
                          <tr>
                              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-neutral-400">Task</th>
                              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-neutral-400">Assigned To</th>
                              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-neutral-400">Due Date</th>
                              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-neutral-400">Status</th>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200 dark:bg-neutral-800 dark:divide-neutral-700">
                          {filteredTasks.map((task: Task) => {
                              ...
                              const dueDateInfo = getDueDateStatus(task.dueDate);
                              const rowColor = dueDateInfo.status === 'overdue' ? 'bg-red-50 dark:bg-red-900/20' : dueDateInfo.status === 'due-soon' ? 'bg-yellow-50 dark:bg-yellow-900/20' : '';
                              const dependencyNames = task.dependencyIds?.map(id => taskMap[id]?.title).filter(Boolean).join(', ');
                              return (
                                  <tr key={task.id} className={rowColor}>
                                      <td className="px-6 py-4 whitespace-nowrap">
                                          <div className="flex items-center">
                                            <span className="text-sm font-medium text-gray-900 dark:text-neutral-200">{task.title}</span>
                                            {dependencyNames && (
                                          ...
                                          <div className="text-sm text-gray-500 truncate max-w-xs dark:text-neutral-400">{task.description}</div>
                                      </td>
                                      <td className="px-6 py-4 whitespace-nowrap">
                                          {contact ? (
                                              <div className="flex items-center">
                                                  ...
                                                  <div className="ml-4">
                                                      ...
                                                      <div className="text-sm text-gray-500 dark:text-neutral-400">{contact.company}</div>
                                                  </div>
                                              </div>
                                          ) : <span className="text-sm text-gray-500 dark:text-neutral-400">Unassigned</span>}
                                      </td>
